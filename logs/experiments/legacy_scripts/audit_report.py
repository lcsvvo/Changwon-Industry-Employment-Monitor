"""Generate the evidence-linked independent audit report from executed artifacts."""
import hashlib
import json
import sys
...
from model import audit_tools as a, robust, revalidation_phase5 as p5
...
def table(df):
    def clean(x):
        if isinstance(x,float): return '' if np.isnan(x) else f'{x:.4f}'
        return str(x).replace('|',' / ').replace('\n',' ')
    return '\n'.join(['| '+' | '.join(map(str,df.columns))+' |','|'+'|'.join(['---']*len(df.columns))+'|']+
        ['| '+' | '.join(clean(x) for x in row)+' |' for row in df.itertuples(index=False,name=None)])


def main():
    s=json.loads((OUT/'summary.json').read_text())
    spaces=read('parameter_spaces'); metrics=read('revision_metrics'); cert=read('continuous_space_certificates').fillna('')
    latest=read('action_latest'); changes=read('alternative_changed_cases'); effects=read('criterion_decision_effects')
    hist=read('history_recalculation')
    missed=cert[cert.sample_possible!=cert.witnessed_possible][['industry','quarter','sample_possible','witnessed_possible']]
    unresolved=cert[cert.full_space_status=='UNRESOLVED_OUTER_BOUND'][['industry','quarter','witnessed_possible','outer_possible']]
    counts=changes.groupby('model').agg(changed_rows=('industry','size'),check_membership_changes=('check_membership_changed','sum')).reset_index()
    doc,_,panel,base,scenarios,candidates=p5._load_context(ROOT)
    from model import qp_calibration
    old_point=a.fast_matrix(qp_calibration.values_from_panel(panel),base['scenarios'][0]['profiles'],
                           [a.sample_record(scenarios['기준'],candidates['v1.0_crisp'])])[0]
    point=read('point_decisions')
    before_after=point[['industry','quarter']].copy()
    before_after['before']=old_point; before_after['after']=point['v1.0_crisp']
    before_after['changed']=before_after.before!=before_after.after
    before_after.to_csv(OUT/'before_after_assignment.csv',index=False,encoding='utf-8-sig')
    distribution=pd.DataFrame([{'stage':name,'before':int((old_point==code).sum()),
        'after':int((point['v1.0_crisp']==code).sum())} for code,name in [(-1,'UNDETERMINED'),(0,'OBSERVE'),(1,'CHECK'),(2,'PRIORITY')]])
    with_vrc=read('sets_history_corrected_with_vrc4')
    without_vrc=read('sets_history_corrected_without_vrc4')
    vrc_latest=with_vrc.merge(without_vrc,on=['industry','quarter'],suffixes=('_with','_without'))
    vrc_latest=vrc_latest[vrc_latest.quarter==vrc_latest.quarter.max()][['industry','possible_stages_with','possible_stages_without']]
    weight_spaces=spaces[spaces.version=='history_corrected'][['space','n','lambda_mean','w_g1_mean','w_g2_mean','w_g3_mean','w_g4_mean','sample_singleton_share_disc','mean_width']]
    effect_summary=effects.groupby(['criterion','mode']).changed.sum().unstack().reset_index()
    block_points=metrics[(metrics.metric=='point_retention') & (metrics.revision=='joint_blocks2')]['all']
    clean_points=metrics[(metrics.metric=='point_retention') & (metrics.revision=='exclude_known_2024Q3_file_error')]['all']
    block_range=f'{block_points.min():.3f}~{block_points.max():.3f}'
    clean_range=f'{clean_points.min():.3f}~{clean_points.max():.3f}'
    test_record=[]
    for filename in ('baseline_tests.xml','modified_tests.xml','final_tests.xml'):
        path=OUT/filename
        if path.exists():
            suites=ET.parse(path).getroot().iter('testsuite')
            for suite in suites:
                attrs=suite.attrib
                test_record.append({'record':filename,'tests':int(attrs.get('tests',0)),
                    'failures':int(attrs.get('failures',0)),'errors':int(attrs.get('errors',0)),
                    'skipped':int(attrs.get('skipped',0)),'seconds':float(attrs.get('time',0))})
    vintage=pd.read_csv(ROOT/'outputs/tables/vintage_수정폭_실측.csv')
    joint=vintage.pivot(index='분기',columns=['업종','변수'],values='수정율pct').dropna().sort_index()
    ordinals=pd.PeriodIndex(joint.index,freq='Q').asi8
    donor_rows=[]
    for length in (1,2,4):
        for start in range(len(joint)-length+1):
            if length==1 or np.all(np.diff(ordinals[start:start+length])==1):
                donor_rows.append({'block_length':length,'start':joint.index[start],
                    'end':joint.index[start+length-1],'includes_known_file_error':'2024Q3' in joint.index[start:start+length]})
    donors=pd.DataFrame(donor_rows)
    donors.to_csv(OUT/'revision_donor_blocks.csv',index=False,encoding='utf-8-sig')
    case_rows=[
        ...
        ('필연배정 명칭','CONFIRMED','추론','치명적',f"표본 공통 배정을 필연/확정으로 표기. 실제 feasible witness가 표본 누락 배정 {s['sample_missed_categories']}건을 확인.",'continuous_space_certificates.csv; feasible_witnesses.json'),
    ...
    findings=pd.DataFrame(case_rows,columns=['지적','판정','구분','심각도','근거','파일'])
    findings.to_csv(OUT/'findings.csv',index=False,encoding='utf-8-sig')
    experiments=pd.DataFrame([
        ...
        ('E02','같은 원계열 재사용·비교 가능한 과거 고용 이력',f"g4 {s['history_g4_changed_rows']}행 변경; lag 값 일관성 테스트 통과",'계산 정합성 수정 채택','history_recalculation.csv'),
        ...
        ('E05','연속공간 LP 외부범위+feasible witnesses',f"{s['continuous_certified_rows']}/160 집합 확인, {s['continuous_unresolved_rows']}행 미해결, 표본 누락 {s['sample_missed_categories']}건",'표본 필연 용어 폐기; 미해결은 보수적 범위','continuous_space_certificates.csv'),
        ...
        ('E10','2분기 공동 블록·역방향 개정',f'여러 donor 블록; 원값/lag 일관성; 유지율 {block_range}','가정 민감도 유지, 현실 확률로 해석 안 함','revision_metrics.csv joint_blocks2'),
        ('E11','알려진 2024Q3 오류정정 제외',f'유지율 {clean_range}; 남은 고용 개정 2건뿐','통상개정 근거 부족; 본 스트레스 결과를 대체하지 않음','vintage_by_quarter.csv'),
    ...
    ],columns=['실험','목적','관측 결과','판단','근거 파일'])
    experiments.to_csv(OUT/'experiments.csv',index=False,encoding='utf-8-sig')
    # Recreate explicit conditional CAI artifact without pretending witnesses have prior mass.
    doc,_,panel,base,_,_=p5._load_context(ROOT)
    samples=json.loads((OUT/'baseline/samples.json').read_text())
    mx=a.fast_matrix(hist[[j+'_after' for j in a.CRIT]].rename(columns={j+'_after':j for j in a.CRIT}),base['scenarios'][0]['profiles'],samples)
    mask=np.load(OUT/'baseline/space.npz')['mask']
    cai=robust.class_acceptability(mx[mask],panel[['industry','quarter']])
    cai['scope']='finite_parameter_sample; CAI zero does not exclude a feasible boundary assignment'
    cai.to_csv(OUT/'parameter_sample_cai.csv',index=False,encoding='utf-8-sig')
    # One minimal, explicitly descriptive comparison table, not a replacement pass/fail gate.
    piv=metrics[metrics.metric=='point_retention'].pivot(index='revision',columns='model',values='all').reset_index()
    inst=metrics[(metrics.model=='compatible_space') & metrics.metric.isin(['compatible_set_jaccard','compatible_point_containment'])][['revision','metric','all','disc']]
    text=f'''# 독립 감사·수정·재실험 보고서
...
3. 교란은 g를 독립적으로 흔든 것이 아니라 원 수준을 흔들었지만, 같은 분기의 원값과 lag에 다른 개정을 적용했다. 이를 고치고 **비교 가능한 과거 고용 이력**으로 g4를 계산하니 **{s['history_g4_changed_rows']}행**의 지속기간이 바뀌었다. 재분류 위험 기간에서 런을 끊고 Q1 상태는 보존했다.
4. 개정 pool의 큰 고용 수정은 알려진 **2024Q3 원파일 오류 정정**에 집중됐다. 이를 제외한 시나리오에서 유지율은 {clean_range}이지만, 표본이 부족해 일반적인 안정성을 입증하지 못한다. 오류 정정을 포함한 스트레스도 함께 남겼다.
5. 연속공간 외부범위 LP와 실제 feasible witness로 **{s['continuous_certified_rows']}/160행**의 집합을 확인했다(수치 허용오차 조건). **{s['continuous_unresolved_rows']}행**은 미해결로 남겼다. 기존 표본이 놓친 배정 **{s['sample_missed_categories']}건**이 있어 ‘표본 공통=필연’은 실제로 틀렸다.
...
{table(pd.DataFrame(test_record))}
...
점 단계가 바뀐 행은 {int(before_after.changed.sum())}개다. [행별 전후 비교](before_after_assignment.csv)에 전부 남겼다. 고용 이력을 더 이용해 점 단계가 바뀌었다는 사실을 정확도 향상으로 해석하지 않는다. 후단 g4의 기간 정의 수정이며 기존 Q1 국면·Q2 규모·Q3 분석창 국면 런은 보존했다.
...
{table(latest[['industry','q1_state','g1','g2','g4','parameter_stages','revision_scenario_stages','action_group']])}
...
- {s['continuous_unresolved_rows']}행은 전체 q,p 연속공간의 정확한 집합이 아직 확인되지 않았다. 81개 끝점 q,p 조합에서도 해결되지 않은 것으로, 해가 없다는 증명이 아니다.
...
**“연구진 규범을 구현한 것 이상의 실질적 정보가 있는가?”** 입력 자료와 개정 기록에는 실질적 정보가 있다. 모형은 그 정보의 충돌과 규범 의존성을 정리하지만 새로운 독립 증거를 생성하지 않는다. LP 검증이 표본 누락 {s['sample_missed_categories']}건을 찾아낸 것과 개정 오류의 집중을 드러낸 것은 정보의 한계를 더 정확히 표현한 성과다. 정책적 타당성이 추가로 입증된 것은 아니다.

**“단순 규칙보다 더 적절한 업종·더 정직한 불확실성·더 안정적인 대응이 가능해졌는가?”** 더 적절한 업종 선정은 현장 기준이 없어 확인하지 못했다. 안정성 우위도 작고 방식 의존적이어서 채택 근거가 아니다. 불확실성 표현은 실제로 개선됐다. 표본 공통을 확정으로 부르지 않고, 누락 배정을 추가하고, 두 개정 유형과 미해결 {s['continuous_unresolved_rows']}행을 구분했다. 이 개선은 ELECTRE만의 고유 장점이 아니라 단순 규칙에도 적용 가능한 검증 절차의 장점이다.
'''
    (OUT/'REPORT.md').write_text(text,encoding='utf-8')
    inputs={}
    for folder in ('src/model','config'):
        for path in (ROOT/folder).glob('*'):
            if path.is_file(): inputs[path.relative_to(ROOT).as_posix()]=hashlib.sha256(path.read_bytes()).hexdigest()
    for path in (ROOT/'src/run_independent_audit.py',ROOT/'scripts/audit_report.py',ROOT/'data/processed/model/electre_input_panel.csv',ROOT/'data/processed/kicox/changwon_industry_master.csv',ROOT/'outputs/tables/vintage_수정폭_실측.csv'):
        inputs[path.relative_to(ROOT).as_posix()]=hashlib.sha256(path.read_bytes()).hexdigest()
    import scipy
    metadata={'generated_at':datetime.now(timezone.utc).isoformat(),'python':sys.version,'numpy':np.__version__,
        'pandas':pd.__version__,'scipy':scipy.__version__,'parameter_seed':2026,'revision_seed':99,
        'replicates':400,'result_scope':'development_data_not_unseen_validation','verdict':'HYBRID','input_and_code_sha256':inputs}
    (OUT/'run_metadata.json').write_text(json.dumps(metadata,ensure_ascii=False,indent=2),encoding='utf-8')
    print('Report generated',OUT/'REPORT.md')
    print('Missed assignments\n',missed.to_string(index=False))
    print('Rule differences\n',counts.to_string(index=False))
    print('Latest groups\n',latest[['industry','parameter_stages','revision_scenario_stages','action_group']].to_string(index=False))
