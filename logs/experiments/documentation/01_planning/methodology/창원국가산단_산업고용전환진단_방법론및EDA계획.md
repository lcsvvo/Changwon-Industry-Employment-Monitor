...
4. 생산자물가지수(PPI) 대비 명목생산액 디플레이트 — 'PPI 조정 생산'(가격효과를 일부 제거한 잠정 지표) 병행 산출 여부 결정. 물리적 생산량이 아니므로 '실질 생산액'으로 단정해 쓰지 않는다
...
2. 가동업체수 변화와 업체당 평균고용 변화를 산술 분해 — 고용감소가 "가동업체수 변화"와 "업체당 고용 변화" 중 어느 쪽에 더 대응하는지 표시. 산술 항등식의 분해이며 어느 쪽이 원인인지는 이 자료로 확정되지 않는다
...
print(decomp)
# employment_yoy ≈ num_operating_yoy + emp_per_firm_yoy (근사). 산술 분해이며 원인 판정이 아니다
```
