...
- **행정 AI 비서:** 현재 배포본은 Gemini가 연결되어 등록 진단을 설명하고 공식 정책문서를 RAG로 검색하며 근거 유형·출처와 실제 사용 provider를 보여준다. Gemini가 작성·요약한 문장은 숫자·판정·인용 검사를 통과해야 하며, 실패하면 등록 진단·검색 결과로 되돌아간다. Gemini가 연결되지 않은 실행에서는 `진단 근거 도우미·규칙 기반 응답`으로 표시한다. AI는 분석을 재계산하거나 정책을 자동 결정하지 않는다.
