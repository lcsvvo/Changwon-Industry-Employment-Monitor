...
`분석·시스템·배포 완료` · [Streamlit 앱 실행](https://changwon-industry-employment-monitor-cyfkaj7ycpbpdxofbxcjkt.streamlit.app/) · `Python 3.11+`

<p align="center">
  <img src="assets/streamlit-industry-diagnosis-2026Q2-final.png" alt="Gemini가 연결된 배포 Streamlit 앱의 2026년 2분기 점검 대기열과 행정 AI 비서" width="1100">
</p>
<p align="center"><sub>실제 배포 화면 — 2026년 2분기 점검 대기열, 기계 업종 진단카드와 행정 AI 비서</sub></p>
...
- **Recruitment Evidence:** 고용24 공개공고의 기업·직무·경력·지역 정보를 기업·공장등록·업종과 품질을 표시하며 연결한다. 상세 HTML에서 정제한 직무·기술·자격·경력은 선택 업종 안에서 RAG로 검색해 **어떤 기업에 무엇을 물을지** 구체화한다. 공고 수는 업종 전체 노동수요나 미충원 규모가 아니다.
- **정책·기관 Routing:** RAG의 주목적은 진단에서 나온 지원 필요 기능을 현재의 기존 정책에 연결하는 것이다. `LLM/sources`의 공식 HTML·PDF를 검색하고, 기업마당 공식 API의 접수 중 공고와 담당기관·접수 경로를 차례로 확인한다. 담당자가 실제 적합성과 접수 상태를 판단하고 인계를 기록하며, `공모전 팀 제안`은 현재 시행 중인 공식 사업과 구분한다.
- **행정 AI 비서:** Gemini는 등록 진단과 RAG 검색 근거를 바탕으로 설명·요약한다. 공식 정책 RAG와 고용24 채용공고 HTML RAG, 기업마당 최신 공고 조회, 필요 시 Google 공식기관 최신검색의 사용 여부와 실제 사용 경로를 화면에 표시한다. Gemini 문장은 숫자·판정·인용 검사를 통과해야 하며 실패하면 등록 진단·검색 결과로 되돌아간다. AI는 분석을 재계산하거나 정책을 자동 결정하지 않는다.

## 6. 구현 화면과 주요 산출물

| 화면 | 담당자가 할 수 있는 일 |
|---|---|
| **업종 진단** | 분기·업종 선택, 점검 대기열·5초 요약, Q1~Q3·Triage 근거, 최근 판정 추이, 외부·채용 신호, 확인 질문, 진단서 HTML·JSON 저장 |
| **점검 관리** | 점검 후보·진행 중·종결 구분, 점검건 개설, 기업·현장 확인, 지원 필요 기능·결정·인계·다음 검토 분기 기록 |
| **정책·지원 연계** | 진단 지원기능 → 공식 정책 원문 RAG → 기존 지원제도 → 기업마당 현재 공고 → 담당기관·접수 경로 확인, 별도 팀 제안 열람 |
...
> **공개 배포본 안내:** 현재 앱은 사용자 인증이 연결되지 않은 프로토타입이다. 실제 개인정보나 기업 비공개정보를 입력하지 않아야 한다. 기본 로컬 SQLite 업무기록은 배포 환경의 재시작·재배포 시 영구 보존을 보장하지 않는다.

<p align="center">
  <img src="assets/streamlit-industry-diagnosis-2026Q2-evidence.png" alt="배포된 Streamlit 앱의 2026년 2분기 기계 업종 상세 진단카드와 판정 근거 화면" width="1100">
</p>
<p align="center"><sub>상세 진단카드 — Q1~Q3 요약과 E·R·A·P 판정 근거</sub></p>
...
**Q1~Q3, Triage, 외부·채용 근거, 정책·기관 연계, AI/RAG 및 Streamlit 구현·배포는 완료됐다.** 팀명은 **창원해요**, 참가 대상은 **2026년 창원시 AI·데이터 활용 공모전**이다.
