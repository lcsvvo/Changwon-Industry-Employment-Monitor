...
JOBS_DIR = ROOT / "data/raw/employment_center/changwon_jobs"
JOBS = (JOBS_DIR / "changwon_jobs_latest.csv") if (JOBS_DIR / "changwon_jobs_latest.csv").exists() else (JOBS_DIR / "changwon_jobs_20260918.csv")
...
def data_roles() -> pd.DataFrame:
    rows = [
        ...
        ("changwon_jobs", "창원고용복지+센터 공개 채용정보", "CONTEXT", "수집시점의 공개 채용공고", "담당자의 현재 채용수요·직무 확인 단서", "고용 증가, 업종별 구인율 또는 과거 추세", "2026-07-20~09-18 스냅샷, 업종코드·모집인원·역사 패널 없음", "KSIC·모집인원·중복정제된 장기 업종×분기 패널 확보", "data/raw/employment_center/changwon_jobs/changwon_jobs_20260918.csv", "Human Review 조회자료; Triage/업종 수치 미사용"),
...
def jobs_summary(jobs: pd.DataFrame) -> pd.DataFrame:
    summary = jobs.groupby("region", dropna=False).size().rename("posting_count").reset_index()
    ...
    result["interpretation"] = "현재 공개공고 조회용; 공고 수를 고용 증가로 해석하지 않음"
    return result
...
def external_evidence(context: pd.DataFrame, decision: pd.DataFrame, jobs: pd.DataFrame, legal_month: pd.DataFrame, legal_coverage: pd.DataFrame) -> pd.DataFrame:
    ...
    evidence["jobs_note"] = "업종코드·모집인원·역사 시계열이 없어 업종별 수치 신호로 사용하지 않음; 담당자 현재 공고 확인용"
